...
if search:
    with st.spinner("RETRIEVING MATCH DATA..."):
        matches = get_matches(username, tagline, size=num_matches)
        mmr_data = get_mmr(username, tagline)

    if "data" not in matches:
        st.error("Player not found or API error!")
    else:
        total_kills = 0
        total_deaths = 0
        total_assists = 0
        ...
        if not match_results:
            st.warning("No valid matches found! Try increasing match count.")
        else:
            ...
            # --- AI COACHING ---
            st.markdown("<h3 style='color:#ff4655'>◆ AI COACHING TIPS</h3>", unsafe_allow_html=True)
